...
public final class RegUtil {
     ...
     * 移动电话
     */
    public static final String REG_MOBILE_TELEPHONE = "^1\\d{10}$";

    public static final String REG_FIX_PHONE = "^([0-9]{3}-?[0-9]{8}(-?[0-9]{3,4})?)|([0-9]{4}-?[0-9]{7}(-?[0-9]{3,4})?)$";
    /**
     * 合法的名字（字母开头，允许5-16字节，允许字母数字下划线）
     */
    public static final String REG_LEGAL_ACCOUNT = "^[a-zA-Z][a-zA-Z0-9_]{4,15}$";

    public static final String REG_LEGAL_CODE = "^[a-zA-Z0-9]{2,32}$";
    /**
     * i地址
     */
     ...
     * 判断是否为正整数
     */
    private static Pattern integersPattern = Pattern.compile("^[0-9]*$");
    ...
    public static boolean isInteger(String src) {
        boolean returnValue = false;
        if (src != null && src.length() > 0) {
            Matcher m = integersPattern.matcher(src);
            if (m.find()) {
                returnValue = true;
            }
        }
    ...
    public static boolean isFixedLine(String str) {
        ...
        Pattern pattern = Pattern.compile(REG_FIX_PHONE);
